...
public class PaymentDateHandlerTest {
    //planerar att använda denna klass för att göra en ny lista som jämfördatum på payment, alltså få en lista på nuvarande medlemmar, funderade på att ha detta i PersonInfo först...

    FileIO fileIo = new FileIO();
    PaymentDateHandler payment = new PaymentDateHandler();
    String inFile = "src/customers.txt";
    List<PersonInfo> people = fileIo.readData(inFile);         //kallar på readData metoden för att skapa upp listan på alla medlemmar, inklusive tidigare
    ...
    public final void PayingCustomerListTest() {    //här ska jag testa om en metod som jämför datum och skapar en lista av betalande kunder verkligen fungerar
        ...
        LocalDate currentDate = LocalDate.parse("2023-10-17");             //skickar in förbestämt datum pga "kan inte" jämföra aktuell tid i tester
        payingCustomer = payment.payingCustomers(people, currentDate);                    //currentdate är då "tiden fryst" för tester
        ...
        currentDate = LocalDate.parse("2024-02-05");                         //testar i framtiden också! tiden är fryst 2024-02-05
...
//tidigare workprocess och tankar
//tidiga tankar var att skapa upp listan people genom metoden readData i FiloIO, och sedan assert att (genom att räkna manuellt) X mängd person är betalande kunder... Men detta funkar INTE
